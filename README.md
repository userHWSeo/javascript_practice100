...
items에서 문자열을 배열로 바꾸고 숫자로 형변환을 시킨다.

###### 문제 50 - 1

이후 items라는 배열은 bubble함수의 매개변수로 들어가게 되고
<br>
result라는 변수를 만들어 매개변수에 slice()함수를 이용하여 클론(?)을 만든다.
<br>
이렇게 하는 이유는 arr라는 매개변수를 변하지 않게 하기 위해서이다.
<br>
for문을 두 개를 사용하는데 하나는 변수 i 가 result.length - 1 보다 작을 때 까지 반복된다.
<br>
이유는 items의 숫자는 총 6개. 비교하는 횟수는 5번이기 때문이다.
<br>
또 다시 for문을 만들어 이번엔 변수 j 가 result.length - i 보다 작을 때 까지 반복하도록 한다.
<br>
그렇게 j 는 result 배열의 위치를 가르키게 된다.
<br>
마지막으로 if문을 활용하여 result의 [ j ] 번째 요소가 [ j + 1 ] 번째 요소보다 클 경우
<br>
변수 value를 만들어 result [ j ]를 result[ j + 1 ]로 바꾸고
<br>
result [ j + 1 ]을 다시 value로 넣어 오름차순으로 만든다.

###### 문제 50 - 2

두 번째 풀이는 필자가 sort를 활용하여 버블정렬을 활용했는데
<br>
사실 sort 함수를 활용한 오름차순과 내림차순이 더 간단하다.
<br>
뭐 문제에선 for문을 이용하여 구현하라고 명시되어 있으니
<br>
문제를 낸 사람의 의도에서 벗어났기 때문에 스스로 생각만했다.
